...
export async function buildPingCard(latencyMs = 0) {
  ...
  <svg width="1280" height="720" xmlns="http://www.w3.org/2000/svg">
    <style>
      .t{font:700 42px Arial,sans-serif;fill:#fff}.b{font:800 92px Arial,sans-serif;fill:#8dc7ff}
      .h{font:700 26px Arial,sans-serif;fill:#fff}.l{font:600 22px Arial,sans-serif;fill:#c7d9ef}.v{font:700 22px Arial,sans-serif;fill:#fff}
    </style>
...
export async function sendPingCard(sock, jid, msg, latencyMs = 0) {
  const image = await buildPingCard(latencyMs);
  await sock.sendMessage(jid, {
    image,
    mimetype: 'image/png',
    caption: `⚡ *Velocidade:* ${Math.max(0, Math.round(Number(latencyMs) || 0))} ms\n🤖 *Rimuru-bot* está online.`
  }, { quoted: msg });
}
